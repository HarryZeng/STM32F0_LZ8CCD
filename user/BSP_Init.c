...
void TIM3_init(uint16_t Period,uint16_t Prescaler,uint16_t Pulse)
{
	...
	*/
	...
	timer_init_structure.TIM_ClockDivision = TIM_CKD_DIV1;          //ÏµÍ³Ê±ÖÓ,²»·ÖÆµ,24M  
	timer_init_structure.TIM_CounterMode = TIM_CounterMode_Up;      //ÏòÉÏ¼ÆÊýÄ£Ê½  
	timer_init_structure.TIM_Period = Period;                          //Ã¿300 uS´¥·¢Ò»´ÎÖÐ¶Ï,??ADC  
	timer_init_structure.TIM_Prescaler = Prescaler;                      //¼ÆÊýÊ±ÖÓ·ÖÆµ,f=1M,systick=1 uS  
	timer_init_structure.TIM_RepetitionCounter = 0x00;              //·¢Éú0+1µÄupdateÊÂ¼þ²úÉúÖÐ¶Ï 
	...
	TIM_ITConfig(TIM3, TIM_IT_CC4, ENABLE);                       //Ê¹ÄÜTIM3ÖÐ¶Ï
